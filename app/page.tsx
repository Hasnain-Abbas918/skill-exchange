    ...
    >
      <nav className="flex items-center justify-between px-10 py-5 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          >S</div>
          <span className="text-xl font-bold text-white" style={{ fontFamily: "'Syne', sans-serif" }}>SkillSwap</span>
        </div>
        <div className="flex gap-3">
          <Link href="/login" className="text-white/60 hover:text-white font-medium px-4 py-2 rounded-lg transition">Sign In</Link>
          <Link
            href="/register"
      ...
      <section className="flex flex-1 flex-col md:flex-row items-center justify-between px-10 md:px-24 py-20 gap-12">
        <div className="max-w-xl">
          ...
          >⚡ 1,200+ Active Skill Exchanges</div>
          <h1 className="text-5xl md:text-6xl font-extrabold text-white leading-tight mb-6" style={{ fontFamily: "'Syne', sans-serif" }}>
            Exchange
          ...
          <p className="text-white/60 text-lg mb-10 leading-relaxed">
            Teach what you know, learn what you want. No money, no subscriptions — just pure skill exchange.
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
            <Link href="/register" className="text-white font-bold px-8 py-4 rounded-xl text-center transition transform hover:scale-105 shadow-lg" style={{ background: "linear-gradient(135deg, #4f46e5, #06b6d4)" }}>
              Start For Free
            </Link>
            <Link href="/browse" className="font-semibold px-8 py-4 rounded-xl text-center transition border" style={{ background: "rgba(255,255,255,0.07)", borderColor: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.8)" }}>
              Browse Skills →
            </Link>
          </div>
        ...
        <div className="flex-1 grid grid-cols-2 gap-4 max-w-md">
          {[
            ...
            { icon: "🔒", title: "Email Verified", desc: "Every account verified via OTP.", color: "#8b5cf6" },
            { icon: "🌍", title: "100% Free", desc: "No fees, no premium tiers — ever.", color: "#10b981" },
          ].map((card) => (
            <div key={card.title} className="rounded-2xl p-5 border transition hover:border-white/20" style={{ background: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.08)" }}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-3" style={{ background: `${card.color}20` }}>{card.icon}</div>
