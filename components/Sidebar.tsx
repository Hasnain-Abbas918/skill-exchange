  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/5">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white text-lg flex-shrink-0" style={{ background: "linear-gradient(135deg, #4f46e5, #06b6d4)" }}>S</div>
        {!collapsed && <span className="font-bold text-lg text-white" style={{ fontFamily: "'Syne', sans-serif" }}>SkillSwap</span>}
        <button onClick={() => setCollapsed(!collapsed)} className="ml-auto text-white/40 hover:text-white/80 transition text-xs" title={collapsed ? "Expand" : "Collapse"}>
          {collapsed ? "▶" : "◀"}
        </button>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} title={collapsed ? item.label : ""}
              ...
              style={{
                background: isActive ? "linear-gradient(90deg, rgba(79,70,229,0.3), rgba(6,182,212,0.15))" : "transparent",
        ...
        {user?.isAdmin && (
          <Link href="/admin" title={collapsed ? "Admin Panel" : ""}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all"
            style={{ background: pathname.startsWith("/admin") ? "rgba(239,68,68,0.15)" : "transparent", color: pathname.startsWith("/admin") ? "#fca5a5" : "rgba(255,255,255,0.5)" }}>
      ...
      <div className="px-3 py-4 border-t border-white/5">
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <button onClick={handleLogout} disabled={loggingOut}
              className="text-white/30 hover:text-red-400 transition text-sm flex-shrink-0 disabled:opacity-40"
              title="Logout">
              {loggingOut ? "..." : "⏻"}
            </button>
          )}
        ...
        {collapsed && (
          <button onClick={handleLogout} disabled={loggingOut}
            className="w-full mt-3 text-white/30 hover:text-red-400 transition text-sm disabled:opacity-40"
            title="Logout">
            ⏻
          </button>
        )}
